...
for course in soup.find_all('div', class_='courseblock'):
    ...
    credits = clean_text(course.find('p', class_='courseblockcredits').text) if course.find('p', class_='courseblockcredits') else "None"
...
df.to_csv('cs_courses.csv', index=False)
print("Data saved to cs_courses.csv")
